defmodule Logflare.NaturalLanguageLql.AnthropicClient do
  ...
  @default_adapter {Tesla.Adapter.Finch,
                    name: Logflare.FinchDefault, receive_timeout: @receive_timeout}
  @response_schema %{
    "type" => "object",
  ...
  defp complete(api_key, prompt, config) do
    ...
    case Tesla.post(client(api_key, endpoint, config), "/v1/messages", body) do
  ...
  @spec client(String.t(), String.t(), keyword()) :: Tesla.Client.t()
  defp client(api_key, endpoint, config) do
    Tesla.client(
      [
  ...
  @spec normalize_response(Tesla.Env.t(), {:ok, term()} | {:error, Jason.DecodeError.t()}) ::
  ...
  defp normalize_response(%Tesla.Env{status: status}, _decoded) when status in 200..299,
    do: {:error, "Anthropic returned a malformed success response"}

  defp normalize_response(%Tesla.Env{status: status, headers: headers}, decoded) do
    body =
      case decoded do
