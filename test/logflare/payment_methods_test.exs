defmodule Logflare.PaymentMethodsTest do
  ...
  describe "payment_methods" do
    ...
    test "create_payment_method/1 with valid data creates a payment_method" do
      assert {:ok, %PaymentMethod{} = payment_method} = PaymentMethods.create_payment_method(@valid_attrs)
    ...
    test "update_payment_method/2 with valid data updates the payment_method" do
      payment_method = payment_method_fixture()
      assert {:ok, %PaymentMethod{} = payment_method} = PaymentMethods.update_payment_method(payment_method, @update_attrs)
    ...
    test "update_payment_method/2 with invalid data returns error changeset" do
      payment_method = payment_method_fixture()
      assert {:error, %Ecto.Changeset{}} = PaymentMethods.update_payment_method(payment_method, @invalid_attrs)
    ...
    test "delete_payment_method/1 deletes the payment_method" do
      ...
      assert_raise Ecto.NoResultsError, fn -> PaymentMethods.get_payment_method!(payment_method.id) end
    end
