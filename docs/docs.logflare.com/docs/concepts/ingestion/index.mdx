...
Drop fields runs **after** field copying and key-value enrichment, so a value can be copied or used for enrichment first and then dropped from its original location:
