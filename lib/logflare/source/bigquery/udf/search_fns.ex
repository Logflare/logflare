defmodule Logflare.User.BigQueryUDFs.SearchFns do
  ...
  def lf_timestamp_sub(bq_project_id, bq_dataset_id) do
    "
    CREATE OR REPLACE FUNCTION
  `#{bq_project_id}`.#{bq_dataset_id}.LF_TIMESTAMP_SUB(_timestamp TIMESTAMP,
    _interval INT64,
  ...
  def lf_generate_timestamp_array(bq_project_id, bq_dataset_id) do
    "
    CREATE OR REPLACE FUNCTION
    `#{bq_project_id}`.#{bq_dataset_id}.LF_GENERATE_TIMESTAMP_ARRAY(_timestamp1 TIMESTAMP, _timestamp2 TIMESTAMP, _interval INT64, _date_part STRING) AS (
        CASE _date_part
