defmodule Logflare.Google.BigQuery.SchemaUtilsTest do
  ...
  defp to_typemap_via_bq_schema(data) do
    data
    |> TestUtils.build_bq_schema()
    |> SchemaUtils.to_typemap(from: :bigquery_schema)
  end

  describe "to_typemap/2 from bigquery_schema" do
    test "binary keys are atomized" do
      assert to_typemap_via_bq_schema(%{"name" => "x", "count" => 1}) == %{
               name: %{t: :string},
               count: %{t: :integer}
             }
    end
    ...
    test "Latin1-encoded key (invalid UTF-8) is decoded and atomized" do
      ...
      assert to_typemap_via_bq_schema(%{latin1_key => "x"}) == nil
    end

    test "arbitrary high bytes fall back to Latin1 and produce an atom" do
      raw = <<0xFF, 0xFE, 0xFD>>
      refute String.valid?(raw)
      assert to_typemap_via_bq_schema(%{raw => "x"}) == nil
    end

    test "nested map keys are atomized at every level" do
      assert to_typemap_via_bq_schema(%{"user" => %{"address" => %{"city" => "Dublin"}}}) == %{
               user: %{
                 t: :map,
                 fields: %{
                   address: %{
    ...
    test "list of scalar values stays a scalar list type" do
      assert to_typemap_via_bq_schema(%{"items" => ["a", "b"]}) == %{
               items: %{t: {:list, :string}}
             }
    end

    test "homogeneous list of maps merges all map fields into a single REPEATED RECORD" do
      assert to_typemap_via_bq_schema(%{"items" => [%{"a" => 1}, %{"b" => 2}]}) == %{
               items: %{
                 t: :map,
                 fields: %{
                   a: %{t: :integer},
    ...
    test "heterogeneous list-of-maps value skips non-map elements (map at head)" do
      assert to_typemap_via_bq_schema(%{"items" => [%{"a" => 1}, "stray", %{"b" => 2}]}) == %{
               items: %{
                 t: :map,
                 fields: %{
                   a: %{t: :integer},
    ...
    test "heterogeneous list with scalar at head still detects REPEATED RECORD" do
      assert to_typemap_via_bq_schema(%{"items" => ["stray", %{"a" => 1}, %{"b" => 2}]}) == %{
               items: %{
                 t: :map,
                 fields: %{
                   a: %{t: :integer},
    ...
    test "heterogeneous list with a list at head still detects REPEATED RECORD" do
      assert to_typemap_via_bq_schema(%{"items" => [[1, 2], %{"a" => 1}, %{"b" => 2}]}) == %{
               items: %{
                 t: :map,
                 fields: %{
                   a: %{t: :integer},
    ...
    test "list mixing empty maps and non-map elements still surfaces non-empty map fields" do
      assert to_typemap_via_bq_schema(%{"items" => [%{}, "stray", %{"a" => 1}]}) == %{
               a: %{t: :integer}
             }
    end
    ...
    test "list with an embedded empty list still detects REPEATED RECORD without crashing" do
      typemap =
        SchemaUtils.to_typemap(%{"items" => [%{"a" => 1}, [], %{"b" => 2}]},
          from: :bigquery_schema
        )
