...
:::note
In multi-source batches, events that omit `__LF_SOURCE` fall back to the source from the **`source`** or **`source_name`** query string. Omitting both per-event routing and a query-parameter default returns **400** with ingestion errors.
:::
...
5. If set, configured fields will be dropped from the event body. See [drop fields](#drop-fields) for more details.
...
:::note
Destination field names must match the BigQuery column name specification, or the event risks being rejected.
:::
...
A source can be configured to drop specific fields from the event body before it is persisted. This is useful for keys that are needed upstream — for example, by a log shipper for routing — but should not be stored.

One field path per line. Dot syntax addresses nested paths, and `m.` is shorthand for `metadata.`. Missing paths and non-map intermediates are silently ignored.

For example, given a Fluent Bit pipeline that uses `service` and `namespace` for routing but does not need them stored:
...
```json
{ "service": "router", "namespace": "default", "event_message": "hello" }
```
...
Drop fields runs at the end of the transformation pipeline, so a value can be copied or used for enrichment first and then dropped from its original location:

```text
# transform_copy_fields
service:m.routing.service

# transform_drop_fields
service
```

The original `service` key is removed from the top level, but `metadata.routing.service` retains the value.

:::note ClickHouse backends
The ClickHouse backend persists events into fixed OTEL tables via a built-in mapping that already discards unmapped top-level keys. Drop fields is still useful for ClickHouse to keep specific keys out of the catch-all `*_attributes` map columns. Be careful not to drop fields that the mapping reads (e.g. `project`, `trace_id`, `timestamp`) — those will fall back to defaults.
:::
