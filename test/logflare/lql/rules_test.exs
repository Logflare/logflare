defmodule Logflare.Lql.RulesTest do
  ...
  describe "extend_timestamp_range/3" do
    test "replaces a cross-month timestamp range and changes only the previous edge" do
      timestamp_filter = %FilterRule{
        path: "timestamp",
        operator: :range,
    ...
    test "materializes a cross-month shorthand and changes only the next edge" do
      shorthand_filter = %FilterRule{
        path: "timestamp",
        operator: :range,
        values: [~U[2026-01-31 00:00:00Z], ~U[2026-02-01 00:00:00Z]],
        shorthand: "this@day"
      }
      ...
      assert Rules.get_timestamp_filters(result) == [
               %FilterRule{
                 path: "timestamp",
                 operator: :range,
                 values: [~N[2026-01-31 00:00:00], ~N[2026-02-02 00:00:00]],
                 modifiers: %{}
               }
             ]
