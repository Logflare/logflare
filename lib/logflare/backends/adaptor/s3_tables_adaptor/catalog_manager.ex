  ...
  defp warn_on_schema_drift(catalog, event_type, table_name, backend) do
    with {:ok, info} <- Native.table_info(catalog, table_name),
      ...
      {:error, :version_mismatch, error_meta} ->
        metadata = [table_name: table_name] ++ error_meta ++ backend_meta(backend)
        Logger.warning("S3 Tables Iceberg table schema mismatch", metadata)

      {:error, reason} ->
        metadata =
          [error_string: inspect(reason), table_name: table_name] ++ backend_meta(backend)

        Logger.warning("S3 Tables Iceberg table schema check failed", metadata)
    end

    :ok
  end

  defp backend_meta(%Backend{} = backend) do
    [
      backend_id: backend.id,
      backend_uuid: to_string(backend.token),
