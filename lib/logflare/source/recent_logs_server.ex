defmodule Logflare.Source.RecentLogsServer do
  ...
  def handle_info(:touch, state) do
    ...
    latest_event = Backends.list_recent_events_local(source) |> Enum.reverse() |> hd()

    if latest_event do
      now = NaiveDateTime.utc_now()

      if NaiveDateTime.diff(now, latest_event.ingested_at, :millisecond) < @touch_timer do
        Sources.update_source(source, %{log_events_updated_at: DateTime.utc_now()})
      end
    end
