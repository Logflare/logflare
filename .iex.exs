...
if System.get_env("PATH") =~ "google-cloud-sdk" do
  Logger.warn(
    "Google cloud sdk not found on path. Please install at https://cloud.google.com/sdk/docs/install"
  )
end
...
defmodule Scripts do
  ...
  def list_instance_groups do
    [_ | groups] =
      System.cmd("gcloud", ["compute", "instance-groups", "managed", "list"])
      |> elem(0)
      |> String.split("\n")
      |> Enum.map(fn line ->
  ...
  def canary_deploy(instance_group, current_template, canary_template) do
    project = "logflare-232118"
    zone = "europe-west3-b"

    """
    gcloud beta compute instance-groups managed rolling-action start-update #{instance_group} --project=#{project} --type='proactive' --max-surge=1 --max-unavailable=0 --min-ready=300 --minimal-action='replace' --most-disruptive-allowed-action='replace' --replacement-method='substitute' --version=template=#{current_template} --canary-version=template=#{canary_template}},target-size=1 --zone=#{zone}
    """
  end

  def deploy(:staging) do
    ...
    [bin | args] =
      String.split(
        """
      gcloud beta compute instance-groups managed rolling-action replace #{instance_group}--max-surge=1 --max-unavailable=0 --min-ready=300 --replacement-method=substitute --zone=#{zone} --project=#{project}
      """
      )
