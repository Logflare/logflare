#!/usr/bin/env python3
"""Validate and summarize mirrored cross-pipeline Broadway metric sampling logs."""
...
def sampling_evidence(
    ...
    # Parallel processor span contexts are deterministically hashed rather than
    # independently randomized. Keep a strict bound, but allow modest per-block
    # dispersion while the mirrored aggregate remains the primary check.
    tolerance = max(8 * math.sqrt(total_messages * probability * (1 - probability)), expected * 0.05, 1)
...
def parse(
    ...
    if config["scheduler_wall_time"] != "true":
        raise ValueError(f"{path}: scheduler wall time is required")
    if int(config["events"]) % int(config["batch_size"]):
        raise ValueError(f"{path}: fixed work must use complete batches")
        ...
        samples.append(
            {
                ...
                "output_batches": int(sample["output_batches"]),
...
def main() -> None:
    ...
    for path in args.logs:
        ...
        output_batches.extend(int(sample["output_batches"]) for sample in samples)
    ...
    if min(output_batches) != max(output_batches):
        raise SystemExit(
            "output batch count varied across samples: "
            f"{min(output_batches)}..{max(output_batches)}"
        )
    ...
    for condition in CONDITIONS:
        ...
        summary = {
            ...
            "output_batches_per_sample": min(output_batches),
        }
        ...
        print(
            ...
            "reductions_delta_vs_none_percent={:.2f} performance_acceptable={} output_batches_per_sample={}".format(
                condition,
                summary["samples"],
                observation_total,
