defmodule LogflareWeb.SearchLive.FormComponents do
  ...
  def recommended_field_inputs(assigns) do
    ...
    ~H"""
    <div :if={Enum.any?(@fields)} class="form-text">
      <div class="d-flex flex-wrap">
        <div :for={field <- @fields} class="pr-2 pt-1 pb-1">
          ...
          <input id={"#{@id_prefix}[#{field.name}]"} name={input_name(:recommended_fields, field.name)} class="form-control form-control-sm tw-text-xs " type="text" />
        </div>
      </div>
    </div>
    """
  ...
  defp format_recommended_fields(fields) do
    fields
    |> Enum.reduce({[], %{}}, fn field, {order, by_name} ->
      name = Source.query_field_name(field)
      ...
      cond do
        ...
        true ->
          {[name | order], Map.put(by_name, name, %{name: name, required?: required?})}
      end
    end)
    |> then(fn {order, by_name} ->
      order
      |> Enum.reverse()
