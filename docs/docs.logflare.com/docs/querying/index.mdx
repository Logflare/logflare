...
Logflare provides a filtering UI to expressively query for log events. Currently, the [Logflare Query Language](#logflare-query-language) is supported.
...
Chart aggregations rules will display the aggergated trends in the chart. Chart rules use the special prefix `c`
