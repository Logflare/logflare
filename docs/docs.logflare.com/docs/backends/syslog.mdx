...
The Syslog backend is **ingest-only**, and exports logs to a remote Syslog receiver using TCP or TLS. It adheres to [RFC 5424](https://www.rfc-editor.org/rfc/rfc5424) and uses [octet-counting framing](https://www.rfc-editor.org/rfc/rfc6587#section-3.4.1).
...
- `structured_data`: (`string`) Static [RFC 5424 Structured Data](https://www.google.com/search?q=https://datatracker.ietf.org/doc/html/rfc5424%23section-6.3) to include in every log frame (e.g., `[exampleSDID@32473 iut="3" eventSource="Application"]`).
