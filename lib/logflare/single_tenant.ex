defmodule Logflare.SingleTenant do
  ...
  def ensure_supabase_sources_started do
    ...
    if user do
      for source <- Sources.list_sources_by_user(user) do
        if postgres_backend?() || clickhouse_backend?() do
  ...
  @doc """
  Returns the adapter opts for the clickhouse backend
  """
  ...
  def supabase_mode_status do
    ...
    source_schemas_updated =
      cond do
        postgres_backend?() || clickhouse_backend?() -> :ok
  ...
  """
  @spec backend_type :: :postgres | :bigquery | :clickhouse
  def backend_type do
    cond do
