...
| `RDS_CA_CERT_PATH`                             | String, defaults to the container bundle path                           | AWS RDS CA bundle for direct RDS or Aurora IAM replicas. Set it outside container images and for non-commercial AWS partitions.                                                                                                                                    |
...
`auth=iam` replaces the password with an RDS IAM token minted for each connection. IAM connections always use verified TLS; `ssl=false` is rejected and omitted `ssl` is enabled automatically. The entry carries a username but no password:

```
LOGFLARE_READ_REPLICAS=postgres://logflare@my-proxy.proxy-abc.eu-west-1.rds.amazonaws.com:5432/logflare?auth=iam
```

Use the AWS-issued database, cluster, reader, or proxy endpoint. IAM signing includes that hostname, so aliases and tunnels are unsupported. The runtime needs `rds-db:connect` for the database user. ExAws supports static environment credentials (including `AWS_SESSION_TOKEN`), EKS Pod Identity, ECS task roles, and EC2 instance roles.

RDS Proxy uses ACM certificates and the image's system trust store. Direct RDS and Aurora endpoints require an [RDS CA bundle](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.SSL.html). Container images install the commercial-region bundle at `/etc/ssl/certs/aws-rds-global-bundle.pem`; non-container deployments and non-commercial AWS partitions must set `RDS_CA_CERT_PATH`.
