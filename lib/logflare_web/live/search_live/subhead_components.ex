  ...
  def timezone(assigns) do
    ...
    <.form :let={f} for={build_form(assigns)} phx-change="results-action-change" id="results-actions">
      <div class="input-group input-group-sm ">
        <label class="input-group-prepend tw-grow" for={f[:search_timezone].id}>
          <span class="input-group-text tw-py-0 tw-bg-transparent tw-text-black tw-text-xs">display timezone</span>
        </label>
        <div class="tw-w-64">
          <CoreComponents.combobox field={f[:search_timezone]} options={build_timezones_select_form_options()} class="form-control form-control-sm tw-text-xs" />
        </div>
