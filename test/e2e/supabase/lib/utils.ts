...
export async function searchLogs(page: Page, searchText: string, expectedText = searchText): Promise<void> {
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    ...
    if (text && !text.includes('No results found') && text.includes(expectedText)) return;
  }
  ...
  throw new Error(
    `searchLogs: "${expectedText}" did not appear in results for "${searchText}" after ` +
      `${(MAX_RETRIES * RETRY_INTERVAL) / 1_000}s.`,
  );
}
