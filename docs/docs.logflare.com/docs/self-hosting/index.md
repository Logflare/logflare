...
| `LOGFLARE_SIGNUP_ALLOWED_DOMAINS`              | Comma-separated string, defaults to `nil`                               | If set, only an email address on one of these domains can create a new account or join a team as a new member. Example: `supabase.com,supabase.io`. If the variable is not set or is empty, all domains are allowed. The match is exact, so a subdomain needs its own entry. Existing accounts can still sign in. Logflare does not start if an entry is not a domain name. Accounts that the partner API creates and the single-tenant user are not checked. |
