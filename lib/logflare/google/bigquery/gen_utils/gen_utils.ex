defmodule Logflare.Google.BigQuery.GenUtils do
  ...
  def get_project_id(source_id) when is_atom(source_id) do
    %Source{user_id: user_id} = Sources.get_source_by(token: source_id)
  ...
  def get_bq_user_info(source_id) when is_atom(source_id) do
    %Source{user_id: user_id, bigquery_table_ttl: ttl} = Sources.get_source_by(token: source_id)
  ...
  def get_account_id(source_id) when is_atom(source_id) do
    %Logflare.Source{user_id: account_id} = Sources.get_source_by(token: source_id)
    "#{account_id}"
  end
