defmodule LogflareWeb.Live.DisplayHelpers do
  ...
  def sanitize_backend_config(config) when is_map(config) do
    allowed_keys =
      ~w(batch_timeout database default_read_cluster hostname host max_message_bytes pool_size port project_id read_only_url read_only_urls region s3_bucket schema storage_region table url use_async_inserts_for_small_batches async_insert_cluster_url async_insert_max_rows)a
